...
describe("EdgeWorker - PR review trigger gate (CYPACK-1273)", () => {
	...
	it("replies to the root of an inline thread while reacting to the comment that mentioned Miko", async () => {
	...
	it("uses an error result immediately even before the runner stores it", async () => {
		...
		expect(mockGitHubCommentService.postIssueComment).toHaveBeenCalledWith(
			expect.objectContaining({
				body: "The task did not complete successfully. Please check the Miko session logs before retrying.",
			}),
		);
	});
...
- Handle only the triggering request above. Other PR comments and reviews are context, not additional assignments; Miko schedules those requests separately.
- Miko publishes your final answer as the reply to this request. Do not post receipt, progress, or completion comments yourself with gh pr comment, the GitHub API, or MCP tools.
