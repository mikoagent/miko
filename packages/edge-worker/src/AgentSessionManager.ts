...
import {
	...
	type RunnerType,
	type SerializedMikoAgentSession,
	type SerializedMikoAgentSessionEntry,
	type Workspace,
} from "miko-core";
...
export type AgentSessionManagerEvents = {
	sessionRemoving: (session: MikoAgentSession) => void;
};
...
export class AgentSessionManager extends EventEmitter {
	...
	private entries: Map<string, MikoAgentSessionEntry[]> = new Map(); // Stores a list of session entries per each session by its id
	...
	private bufferedAssistantEntryBySession: Map<string, MikoAgentSessionEntry> =
	...
	createMikoAgentSession(
		...
		const agentSession: MikoAgentSession = {
			id: sessionId,
	 ...
	 *
	 * Unlike {@link createMikoAgentSession}, this does NOT require issue
	...
	private async createSessionEntry(
		...
		// Evidence from ~/.miko/logs/CYGROW-348 session jsonl shows assistant messages with
	...
	private async updateSessionStatus(
		...
		additionalMetadata?: Partial<MikoAgentSession["metadata"]>,
	): Promise<void> {
		const session = this.sessions.get(sessionId);
		if (!session) return;
	...
	private async addResultEntry(
		...
		const resultEntry: MikoAgentSessionEntry = {
			// Set the appropriate session ID based on runner type
			...(runnerType === "gemini"
	 ...
	 */
	getSessionEntries(sessionId: string): MikoAgentSessionEntry[] {
	 ...
	 */
	getActiveSessions(): MikoAgentSession[] {
	 ...
	 */
	private getSessionIssueId(session: MikoAgentSession): string | undefined {
	 ...
	 */
	getSessionsByIssueId(issueId: string): MikoAgentSession[] {
		return Array.from(this.sessions.values()).filter(
	 ...
	 */
	getActiveSessionsByIssueId(issueId: string): MikoAgentSession[] {
		return Array.from(this.sessions.values()).filter(
			(session) =>
	 ...
	 */
	getActiveSessionsByBranchName(branchName: string): MikoAgentSession[] {
		return Array.from(this.sessions.values()).filter(
			(session) =>
				session.status === AgentSessionStatus.Active &&
	...
	serializeState(): {
		...
		const sessions: Record<string, SerializedMikoAgentSession> = {};
		const entries: Record<string, SerializedMikoAgentSessionEntry[]> = {};
	 ...
	 */
	restoreState(
		serializedSessions: Record<string, SerializedMikoAgentSession>,
		serializedEntries: Record<string, SerializedMikoAgentSessionEntry[]>,
	): void {
		...
		for (const [sessionId, sessionData] of Object.entries(serializedSessions)) {
			const session: MikoAgentSession = {
				...sessionData,
		...
		for (const [sessionId, entriesData] of Object.entries(serializedEntries)) {
			const sessionEntries: MikoAgentSessionEntry[] = entriesData.map(
				(entryData) => ({
					...entryData,
				}),
			);
