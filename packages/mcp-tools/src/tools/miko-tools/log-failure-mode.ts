 ...
 *   - `sessionId` — miko internal session UUID, used as the dedup key
 ...
 *     identifier (Linear "ENG-76", GitHub PR "miko#1234", GitLab MR,
...
export interface ResolvedSession {
	...
	runnerType?: "claude" | "gemini" | "codex" | "cursor" | "opencode" | "grok" | null;
...
/**
 * HTTP client interface for posting to miko-hosted. Tests can substitute
...
export interface LogFailureModeOptions {
	 ...
	 * server (e.g. parentSessionId passed to `createMikoToolsServer`).
	 */
					...
					content: [
						{
							...
							text: JSON.stringify({
								success: false,
								error: `miko-hosted POST /api/failure-modes failed: ${result.status} ${result.error}`,
							}),
						},
					],
				};
