 ...
 * - Clones repositories to ~/.miko/repos/<repo-name> using GitHub CLI (gh)
 * - If repository exists, verify it's a git repo and do nothing
 * - If repository doesn't exist, clone it to ~/.miko/repos/<repo-name>
 */
export async function handleRepository(
	...
	try {
		...
		// Construct path within repos directory (defaults to ~/.miko/repos, overridable via MIKO_REPOS_DIR)
		const reposDir = getDefaultReposDir(mikoHome);
 ...
 * - Removes repository directory from ~/.miko/repos/<repo-name>
 * - Removes worktrees from ~/.miko/workspaces/<linear-team-key>/<repo-name>
 */
export async function handleRepositoryDelete(
	...
	try {
		...
		if (payload.linear_team_key) {
			const workspacesDir = join(mikoHome, "workspaces");
